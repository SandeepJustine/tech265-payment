...
class ApiMiddleware
{
    ...
    public static function authenticate($requiredRole = 'any')
    {
        ...
        if (strpos($key, 'Bearer ') === 0) {
            $key = substr($key, 7);
        }

        // Also accept ?api_key=... for convenience / testing
        if (!$key && !empty($_GET['api_key'])) {
            $key = $_GET['api_key'];
        }
        ...
        if (!$key) {
            self::error('API key is required. Pass it via X-API-Key header.', 401, [], 'MISSING_API_KEY');
        }
