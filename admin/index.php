...
// Direct charge + payout stats (graceful fallback if tables don't exist yet)
$directChargesTotal   = 0;
$directChargesSuccess = 0;
$payoutsTotal         = 0;
$payoutsDisbursed     = 0;
try {
    $directChargesTotal   = $db->query("SELECT COUNT(*) FROM direct_charges")->fetchColumn();
    $directChargesSuccess = $db->query("SELECT COUNT(*) FROM direct_charges WHERE status IN('success','successful')")->fetchColumn();
    $payoutsTotal         = $db->query("SELECT COUNT(*) FROM payouts")->fetchColumn();
    $payoutsDisbursed     = $db->query("SELECT COALESCE(SUM(amount),0) FROM payouts WHERE status IN('success','successful')")->fetchColumn();
...
<div class="kpi-grid">
  ...
  <div class="kpi-card">
    <div class="kpi-icon" style="background:#e0f2fe">📱</div>
    <div>
      <div class="kpi-val"><?= number_format($directChargesTotal) ?></div>
      <div class="kpi-label">Direct Charges</div>
    </div>
  </div>
  <div class="kpi-card">
    <div class="kpi-icon" style="background:#fce7f3">💸</div>
    <div>
      <div class="kpi-val" style="color:#9d174d">MWK <?= number_format($payoutsDisbursed, 2) ?></div>
